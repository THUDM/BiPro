...
import numpy as np
from ppl import ppl
def compute_ip(model,ini_pos,start_pos,end_pos,gen_pos,sop_pos,beams,tokenizer,enhanced_start=None,enhance_end=None,weight=0.5,fix_st=0):
    #compute new beams
    new_beams=[]
    targets=[]
    ...
    if enhanced_start is not None:
        ...
        #actual_start=max(fix_st,actual_start-5)
...
class iPromptStrategy:
    def __init__(self, num_beams, length_penalty=1., consider_end=False,
                end_tokens=[], invalid_slices=[], no_repeat_ngram_size=0, min_tgt_length=0,verifier=None,verifier_params=None,num_samples=12,factor=0.1,end_factor=0.1,tmp_factor=1.1,act_factor=0.75,mode='chinese'):
    ...
    def _add_end_beams(self, score, verify_score,beam,enhance_start=None,enhance_end=None):
        ...
        if self.mode=='chinese':
            ip_score=compute_ip(self.model,self.ini_pos,self.iprompt_start_pos,self.iprompt_end_pos,self.gen_pos,self.sop_pos,[beam],self.verifier_params[0],enhanced_start=enhance_start,enhance_end=enhance_end,fix_st=self.fix_st)
        if self.mode=='english':
            ip_score=compute_ip(self.model,self.ini_pos,self.iprompt_start_pos,self.iprompt_end_pos,self.gen_pos,self.sop_pos,[beam],self.verifier_params[0],enhanced_start=enhance_start,enhance_end=enhance_end,fix_st=self.fix_st)
        ...
        for i in range(len(self.end_beams), -1, -1):
            if i == 0 or score < self.end_beams_penalized_scores[i-1]:
                break
        #print(self.verifier_params[0].DecodeIds(beam.cpu().tolist()),score,file=logger_file)
    ...
    def forward(self, logits, tokens, mems, use_ip=True,ban_end=False,max_length=375,fixed_next=None):
        ...
        scores_for_sample=next_token_scores*self.tmp_factor+act_scores*self.act_factor
        ...
        while (total_added<self.num_beams):
            ...
            if not(ban_end) and (times==1) and not(fixed_next):
        ...
        if len(beam_continue)>0:
            ...
            if use_ip:
                ...
                iprompt_scores=compute_ip(self.model,self.ini_pos,self.iprompt_start_pos,self.iprompt_end_pos,self.gen_pos,self.sop_pos,beam_continue,self.verifier_params[0],enhanced_start=self.st_pos,enhance_end=current_pos,fix_st=self.fix_st)
