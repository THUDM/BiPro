...
class SingleGenStrategy:
    def __init__(self,  length_penalty=1.,
                end_tokens=[], invalid_slices=[],verifier=None,verifier_params=None,tmp_factor=1.1,bad_tokens=None):
        self.num_beams = 1
    ...
    def forward(self, logits, tokens, mems,fixed_next=None):
        ...
        for token in self.bad_tokens:
            for i in range(batch_size):
                scores_for_sample[vocab_size*i+token]=-1000
        ...
        while (total_added<self.num_beams):
            ...
            next_tokens = torch.multinomial(probs,
                    num_samples=self.num_samples*20) # [2*nb]
            ...
            for i in range(len(next_tokens)):
                if int(next_tokens[i]) in self.bad_tokens:
                    print(scores_for_sample,times)
                if self.is_done:
                    continue
        ...
        print("total_beams:",len(self.end_beams),"beam_continue:",len(beam_continue))
        if len(beam_continue)>0: 
            print('current generation:',beam_continue[0])
